...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['login'])) {
        ...
        if ($stmt->fetchColumn() > 0) {
    ...
    } elseif (isset($_POST['update']) && isset($_SESSION['user_id'])) {
            $first_name = trim($_POST['first_name'] ?? '');
            $last_name  = trim($_POST['last_name']  ?? '');
            $email      = trim($_POST['email']      ?? '');
            $address    = trim($_POST['address']    ?? '');
            ...
            if ($stmt->fetch()) {
                ...
                $stmt = $pdo->prepare("UPDATE users SET first_name = ?, last_name = ?, email = ?, address = ?, city = ?, state = ?, zip_code = ?, country = ?, phone = ? WHERE user_id = ?");
                try {
                    $stmt->execute([$first_name, $last_name, $email, $address, $city, $state, $zip_code, $country, $phone, $_SESSION['user_id']]);
